...
final Vibrator vib=(Vibrator)this.getBaseContext().getSystemService(Context.VIBRATOR_SERVICE);
        final long[][] array = new long[1][1];
        mRealtimeWaveformView = (WaveformView) findViewById(R.id.waveformView);
        mRecordingThread = new RecordingThread(new AudioDataReceivedListener() {
            @Override
            public void onAudioDataReceived(short[] data) {
                mRealtimeWaveformView.setSamples(data);
                array[0] =new long[data.length];
                int index=0;
                for(int i=0;i<data.length;i+=5){
                    array[0][index]=(long)data[i];
                    index++;
                }
            }
        });
...
FloatingActionButton fab2=(FloatingActionButton) findViewById(R.id.vibFab);
        fab2.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                vib.vibrate(array[0],1);
            }
        });
